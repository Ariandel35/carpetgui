...
#       noncompliant, but you have some other POSIX shell available, most
#       probably configure Gradle to use by setting the SHELL environment
#       variable.
#
#   (2) Avoid modifying this script unless you know what you are doing -- all
#       subsequent version of Gradle will try to update this file.
#
##############################################################################
...
# Add Gradle jars to classpath
GRADLE_JAR=$GRADLE_HOME/gradle/wrapper/gradle-wrapper.jar

if [ ! -f "$GRADLE_JAR" ]; then
    echo "ERROR: gradle-wrapper.jar not found in $GRADLE_HOME/gradle/wrapper/" >&2
    exit 1
fi
...
if "$cygwin" || "$msys" ; then
    ...
    for arg do
        ...
        # args, so each arg gets converted only once.
        i=$((i+1))
        eval "set -- \"\$@\" '$arg'"
    done
fi
...
# Now set the args for the java invocation.
# Collect all arguments for the java command, stacking in reverse order:
#   * args from the command line
#   * the main class name
#   * -classpath
